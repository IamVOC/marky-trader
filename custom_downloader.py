...
async def _download_symbols(exchange_name, symbols, dir, timeframe, path_date, **kwargs):
    exchange = getattr(ccxt, exchange_name)({ 'enableRateLimit': True })
    for symbol in symbols:
        df = await _download_symbol(exchange = exchange, symbol = symbol, timeframe= timeframe, **kwargs)
        save_file = f"{dir}/{exchange_name}-{symbol.replace('/', '')}-{timeframe}-{path_date[0].strftime('%Y')}-{path_date[1].strftime('%Y')}.pkl"
